...
parser.add_argument("-v", "--video", required=False, type=str,
                    default='test_images/los_angeles.mp4', help="path to video input file", metavar="FILE")
parser.add_argument("-t", "--tracker", required=False, type=str,
                    default='csrt', help="path to video input file", metavar="FILE")
...
arbiter = Arbiter(args['prototxt'], args['model'], useGPU, args['serial'], args['tracker'], logger) #'mosse')
...
fps.stop()
arbiter.logger.info("Input frame: elasped time: {:.2f}".format(fps.elapsed()))
arbiter.logger.info("Input frame: approx. FPS: {:.2f}".format(fps.fps()))
arbiter.stop()
arbiter.logger.stop()
cap.release()